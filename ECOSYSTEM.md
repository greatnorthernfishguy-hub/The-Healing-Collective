...
Seven files are copied verbatim from NeuroGraph canonical into every module. They are
...
**Current standard (since 2026-03-22):** `Snowflake/snowflake-arctic-embed-m-v1.5` (768-dim)
via `ng_embed.py` (ONNX Runtime + tokenizers). All modules use the same centralized
embedding through the vendored `ng_embed.py` singleton — no per-module embedding code.
Replaced `BAAI/bge-base-en-v1.5` via fastembed (+1.89 MTEB retrieval improvement).

**Dual-Pass Embedding (#81) — Deployed:**
- Pass 1: Gestalt embedding of whole content (the forest — what is this about?)
- Pass 2: LLM-assisted concept extraction via TID → embed each concept individually (the trees — what's in it?)
- `dual_record_outcome()` in `ng_ecosystem.py` — modules opt in per recording call
